    ...
    <main className="min-h-screen">
      ...
      <div className="bg-primary pt-40 pb-20 relative overflow-hidden">
        {service.featured_image && (
          <div className="absolute inset-0 bg-cover bg-center opacity-20 grayscale" style={{ backgroundImage: `url('${service.featured_image}')` }} />
        )}
        <div className="container-custom relative z-10">
          <Link href="/services" className="inline-flex items-center gap-2 text-accent text-xs font-bold uppercase tracking-widest mb-8 hover:text-white transition-colors">
            <ArrowLeft className="w-4 h-4" />
            All Services
          </Link>
          <Heading level={1} className="text-white uppercase italic">{service.title}</Heading>
          {service.short_description && <p className="text-white/70 text-lg mt-6 max-w-2xl">{service.short_description}</p>}
        </div>
      </div>
      <Section>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          <div className="lg:col-span-2">
            {service.description ? (
              <div className="prose prose-lg max-w-none text-neutral-700" dangerouslySetInnerHTML={{ __html: service.description }} />
            ) : (
              <p className="text-neutral-600 leading-relaxed text-lg">
                Multi Green Engineering provides professional {service.title.toLowerCase()} services backed by our CIDB G7 and PKK Class &apos;A&apos; certifications.
              </p>
            )}
          ...
          <div>
            {service.featured_image && (
              <div className="aspect-[4/3] mb-8 overflow-hidden border border-industrial">
                <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url('${service.featured_image}')` }} />
              </div>
            )}
            {otherServices.length > 0 && (
              <div className="bg-primary p-8 mb-8">
                ...
                <div className="space-y-4">
                  {otherServices.map((s) => (
                    <Link key={s.id} href={`/services/${s.slug}`} className="block text-white/70 hover:text-accent transition-colors text-sm font-medium">{s.title}</Link>
                  ))}
                </div>
              </div>
            )}
            <div className="p-8 border border-industrial">
              ...
              <p className="text-neutral-600 text-sm mb-6">Need {service.title.toLowerCase()} for your project? Contact our team for a consultation.</p>
              <Link href="/contact" className="block text-center bg-accent text-white px-6 py-4 font-bold uppercase tracking-widest text-xs hover:bg-primary transition-colors">Contact Us</Link>
            </div>
          </div>
        </div>
      </Section>
